...
public class UtilQueueSocket extends UtilQueue<Socket> {
	...
	public synchronized void handle(Socket socket) {
		...
		try {
			...
			log.info(LogType.thread, this, "线程：读socket：" + Thread.currentThread().toString());
			...
			long applicationId = engineSocket.getCountStore().put(socket);
			...
			if (protocol.equals("WEBSOCKET")) {
				...
				boolean result = engineSocket.getListWebSocketThread().put(webSocket);
				if (result == false) {
					map.put(CoreType.sec_websocket_accept.toString(), "i am sorry");
				} else {
					engineSocket.getPoolWebSocketThread().execute(webSocket);
				}
			}
